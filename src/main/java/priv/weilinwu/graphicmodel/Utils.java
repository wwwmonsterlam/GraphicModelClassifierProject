...
//		logger.debug(matrixs[1].toString());
	...
	public static Matrix[] getTrainingSet(Matrix[] matrixs) {
		...
		trainingSet[0] = matrixs[0].subMatrix(Ret.NEW, 0, 0, rowCount - 1, columnCount / 4 * 3 - 1);
		trainingSet[1] = matrixs[1].subMatrix(Ret.NEW, 0, 0, rowCount - 1, columnCount / 4 * 3 - 1);
	...
	public static Matrix[] getTestingSet(Matrix[] matrixs) {
		...
		testingSet[0] = matrixs[0].subMatrix(Ret.NEW, 0, columnCount / 4 * 3, rowCount - 1, columnCount - 1);
		testingSet[1] = matrixs[1].subMatrix(Ret.NEW, 0, columnCount / 4 * 3, rowCount - 1, columnCount - 1);
